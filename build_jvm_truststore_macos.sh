#!/usr/bin/env bash
# (c) JFrog Ltd. (2026)
# Build a JVM truststore from macOS system keychains plus one custom CA PEM.
#
# This is a build-time helper for the JVM installers. It does not install
# anything and does not require root.
#
# Run:
#   ./build_jvm_truststore_macos.sh --use-cert /path/to/company-ca.pem --output /tmp/package-route-truststore.jks
...
usage() {
    cat <<EOF
Usage:
  $0 --use-cert <path> --output <path> [--cert-alias <alias>]
