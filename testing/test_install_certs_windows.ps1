...
'@
    ...
    Write-Host ""
    Write-Host "=== install_certs_windows.ps1 pip / UV_NATIVE_TLS + Hugging Face Hub flow ==="

    # -UseCert -Package pip: install sets Machine pip + HF Hub vars; verify and clean up
    ...
    try {
        ...
        if ($rPip.ExitCode -ne 0) {
            ...
            if ($uv -eq "1" -and $req -eq $CertPath -and $hfx -eq "1" -and $hfe -eq "86400" -and $hfd -eq "86400") {
                Write-Host "  OK ($Run): -Package pip sets UV_NATIVE_TLS, REQUESTS_CA_BUNDLE, HF_HUB_*"
                $script:Pass++
            } else {
    ...
    # -UseCert -Package all: verify npm and pip vars set (including HF Hub)
