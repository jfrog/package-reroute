...
$installedHash = (Get-FileHash -LiteralPath $JksPath -Algorithm SHA256).Hash
...
if (-not ($jto -like "*trustStore=`"$JksPath`"*") -and -not ($jto -like "*trustStore=$JksPath*")) {
    Fail-Test ("JAVA_TOOL_OPTIONS doesn't reference installed JKS after re-install. got: {0}" -f $jto)
}
